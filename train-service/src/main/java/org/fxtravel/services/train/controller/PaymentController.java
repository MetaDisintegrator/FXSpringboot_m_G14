...
@RestController
@RequestMapping("/api/train/payment")
public class PaymentController {
    ...
    public ResponseEntity<Boolean> completePayment(@RequestHeader("X-User-Id") String userId, @RequestBody PaymentRequest request) {
        boolean result = paymentService.completePayment(request.getOrderNumber(), request.getData());
        return ResponseEntity.ok(result);
    }

    @PostMapping("/fail")
    public ResponseEntity<Boolean> cancelPayment(@RequestHeader("X-User-Id") String userId, @RequestBody PaymentRequest request) {
        boolean result = paymentService.failPayment(request.getOrderNumber(), request.getData());
        return ResponseEntity.ok(result);
    }

    @PostMapping("/finish")
    public ResponseEntity<Boolean> finishPayment(@RequestHeader("X-User-Id") String userId, @RequestBody PaymentRequest request) {
